...
<div class="container">
    <div class="card">
        <h2 class="card-title"><i class="fas fa-chart-line"></i> Top 5 — Médecins les plus consultés</h2>
        <p style="color:var(--text-secondary); font-size:13px; margin-bottom:20px;">
            <i class="fas fa-chart-simple"></i> Classement des médecins les plus sollicités par les patients
        </p>
                    ...
                    <div class="top5-card" style="display:flex; align-items:center; gap:16px; padding:16px;
                        ...
                        <div style="flex:1;">
                            <h3 style="font-size:18px; color:#1a73e8; margin-bottom:4px;">
                                <i class="fas fa-user-md"></i> Dr. ${m.nommed}
                            </h3>
                            <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center;">
                                <span class="badge badge-success"><i class="fas fa-stethoscope"></i> ${m.specialite}</span>
                                <span style="color:var(--text-secondary); font-size:13px;"><i class="fas fa-map-marker-alt"></i> ${m.lieu}</span>
                            </div>
                            <div style="margin-top:8px;">
                                <span style="color:var(--text-secondary); font-size:13px;"><i class="fas fa-money-bill-wave"></i> Taux horaire: </span>
                                <strong style="color:#1a73e8;">${m.tauxHoraire} Ar/h</strong>
                            </div>
                        </div>

                        <div class="consult-badge" style="font-size:13px; color:#34a853; background:#e6f4ea; padding:8px 16px; border-radius:20px; display:flex; align-items:center; gap:8px;">
                            <i class="fas fa-star"></i> <span>Médecin référent</span>
                        </div>
                    </div>
                </c:forEach>

                <div style="margin-top:25px; padding:15px; background:var(--hover-bg); border-radius:10px; text-align:center;">
                    <i class="fas fa-heart" style="color:#ea4335;"></i> <span style="font-size:14px;"> Excellents médecins dans le classement !</span>
                </div>
            </c:otherwise>
        </c:choose>
        ...
        <div style="margin-top:25px; text-align:center;">
            <c:choose>
                <c:when test="${sessionScope.role == 'admin'}">
                    <a href="${pageContext.request.contextPath}/admin?action=medecins" class="btn btn-primary">
                        <i class="fas fa-search"></i> Voir tous les médecins
                    </a>
                </c:when>
                <c:otherwise>
